...
**REALGenomeSIM** simulates GWAS data with **R**ealistic **E**pistatic effects, **A**dditive effects, and **L**D patterns with **Genome** **S**ubsampling and **I**ntegrated **M**odeling. Each genome that REALGenomeSIM simulates starts out as an empty template of SNP positions without SNP values, which is divided into empty segments that are demarcated by breakpoints. The probability of drawing any genomic position for a given breakpoint is equal to the probability that this position would demarcate a given real recombination event. Once an empty simulated genome is segmented by breakpoints, the indices of whole genomes from a real dataset are duplicated so that 1) there is one real individual for each empty segment and 2) every real individual is selected an equal number of times (minus the remainder if the number of segments is not divisible by the number of individuals). Then, for each empty segment, an individual is randomly selected without replacement from the set of duplicated individuals, and the empty segment is filled with the the homologous segment from the sampled real individual. This process is repeated until every segment in the simulated genome is filled with a homologous segment from a real individual. This quasirandom selection has the benefit minimizing maf variation between the simulated and real datasets, while also maintaining normal population level genetic variability by randomizing segment selection. Furthermore, the fact that two randomly selected segments are independent minimally (if at all) disrupts the original LD pattern because each breakpoint location is selected with the same probability that they would demarcate a given real recombination event.
