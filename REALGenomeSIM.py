...
from REALGenomeSIM_library import SNP_positions_to_rcmb_intervals
from REALGenomeSIM_library import reduce_recomb_rate_info
from REALGenomeSIM_library import choice_with_periodic_replacement
from REALGenomeSIM_library import centimorgans_to_probabilities
from REALGenomeSIM_library import draw_breakpoints
from REALGenomeSIM_library import get_samples_fast
from REALGenomeSIM_library import breakpoints_to_simulated_individuals
from REALGenomeSIM_library import write_bed_file
from REALGenomeSIM_library import simulate_phenotypes
...
if num_breakpoints < 0:
    ...
    exit()
if num_breakpoints == 0:
    print("\nwarning: setting 0 breakpoints means that all simulated individuals will be real individuals from the input dataset.\n")
if num_breakpoints > 10:
...
if rcmb_path_prefix != "empty":
    ...
    if pop_code in hg_pop_codes:
        ...
        try:
            ...
            correct_info_file_names = np.sort([pop_code + "_recombination_map_hapmap_format_" + hg_version + "_chr_" + str(i) + ".txt" for i in range(1, 23)])
        except:
...
rcmb_rate_info_paths = [rcmb_rate_info_path_prefix + str(chr) + '.txt' for chr in chromosomes]
rcmb_rate_info_chr = [pd.read_csv(path, delim_whitespace = True, header = 0) for path in rcmb_rate_info_paths]
